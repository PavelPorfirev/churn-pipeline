...
def generate_synthetic_churn(n_samples: int = 20_000, random_state: int = 42) -> pd.DataFrame:
    """
    ...
    df["contract"] = rng.choice(["month-to-month", "one-year", "two-year"], size=n_samples, p=[0.5, 0.3, 0.2])
    df["target"] = y
    return df


def load_split(df: pd.DataFrame, test_size: float = 0.2, random_state: int = 42) -> tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    """
